...
function Card(props) {
  ...
  return (
    <section ref={cardContainer} className={card.vendor.length > 0 ? card.vendor : "allCards-section"} id="allCards-section" onClick={ notCardStack ? null : cardSectionFunction }>
    <button onClick={removeCardBtn} style={ notCardStack ?  null : {display:"none"} }className="remove-btn">X</button>
    ...
    <p>{ card.cardNumber.length > 0 ? card.cardNumber : "XXXX XXXX XXXX XXXX"}</p>
    </div>
